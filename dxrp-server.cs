...
static class Launcher
{
    ...
    const string RepositoryBranch = "legacy";
    ...
    public static async Task Run(string[] args)
    {
        ...
        while (true)
        {
            ...
            if (!devMode)
            {
                ...
                Info(hasRepo ? $"Fetching {RepositoryBranch}..." : $"Cloning {config.RepoUrl} ({RepositoryBranch})...");
                if (hasRepo)
                {
                    RunGit(["-C", RepoDir, "fetch", "--depth", "1", "origin", RepositoryBranch]);
                    RunGit(["-C", RepoDir, "reset", "--hard", "FETCH_HEAD"]);
                }
                else
                {
                    RunGit(["clone", "--depth", "1", "--branch", RepositoryBranch, config.RepoUrl, RepoDir]);
                }
                Ok("Repository ready.");
            }
