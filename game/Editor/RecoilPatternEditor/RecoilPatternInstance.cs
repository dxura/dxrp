...
public class RecoilPatternInstance : GraphicsItem
{
	...
	public RecoilPatternInstance( RecoilPatternEditor editor )
	{
		HoverEvents = true;
		Clip = true;
		Editor = editor;
	...
	Vector2 GetRemappedPoint( Vector2 point )
	{
		...
		var remappedX = x.Remap( RangeX.x, RangeX.y );
		var remappedY = y.Remap( RangeY.x, RangeY.y );
	...
	protected override void OnMousePressed( GraphicsMouseEvent e )
	{
		if ( e.LeftMouseButton )
		{
			...
			if ( Keys.FirstOrDefault( x => x.Hovered ) is not null )
				return;
